...
export default function DitchJetLag() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FFFBF7", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,300;0,9..40,400;0,9..40,500;0,9..40,600;0,9..40,700;0,9..40,800;1,9..40,300;1,9..40,400&family=Space+Mono:wght@400;700&family=Playfair+Display:ital,wght@0,400;0,500;0,600;0,700;1,400;1,500&display=swap');
      ...
      <nav style={{
        ...
        <div style={{ maxWidth: 1140, margin: "0 auto", padding: "0 32px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div className="nav-mid" style={{ display: "flex", gap: 28 }}>
            <span className="nav-link" onClick={() => scrollTo("calculator")}>Calculator</span>
            <span className="nav-link" onClick={() => scrollTo("tips")}>Tips</span>
            <span className="nav-link" onClick={() => scrollTo("articles")}>Articles</span>
          </div>

          <button className="btn-primary" style={{ padding: "10px 22px", fontSize: 11 }} onClick={() => scrollTo("calculator")}>Ditch It →</button>
        </div>
      </nav>
      ...
      {/* ─── CALCULATOR ─── */}
      <section id="calculator" style={{ maxWidth: 760, margin: "0 auto", padding: "0 32px 80px", marginTop: -10, scrollMarginTop: 80 }}>
        <div style={{
      ...
      <section id="articles" style={{ padding: "48px 32px 80px", background: "linear-gradient(180deg, #FFFBF7, #FFF5EE 50%, #FFFBF7)", scrollMarginTop: 80 }}>
        <div style={{ maxWidth: 1140, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 40, flexWrap: "wrap", gap: 16 }}>
            ...
            <button className="btn-outline" style={{ padding: "12px 24px", fontSize: 11 }} onClick={() => scrollTo("faq")}>FAQ →</button>
          </div>
      ...
      {/* ─── FAQ ─── */}
      <section id="faq" style={{ maxWidth: 680, margin: "0 auto", padding: "60px 32px 80px", scrollMarginTop: 80 }}>
      ...
      <footer style={{ borderTop: "2px solid #F0E8E0", padding: "48px 32px 32px", background: "#FFFBF7" }}>
        <div style={{ maxWidth: 1140, margin: "0 auto" }}>
          <div className="footer-cols" style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 40, marginBottom: 40 }}>
            ...
            {[
              { title: "Tool", links: [{ label: "Calculator", id: "calculator" }, { label: "Tips", id: "tips" }, { label: "FAQ", id: "faq" }] },
              { title: "Learn", links: [{ label: "Jet Lag Science", id: "tips" }, { label: "Articles", id: "articles" }, { label: "FAQ", id: "faq" }] },
            ].map((col, i) => (
              <div key={i}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{col.links.map(l => <a key={l.label} href={`#${l.id}`} onClick={(e) => { e.preventDefault(); scrollTo(l.id); }}>{l.label}</a>)}</div>
              </div>
            ))}
          </div>
          <div style={{ borderTop: "2px solid #F0E8E0", paddingTop: 20, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 18 }}>{["Twitter", "Instagram", "Pinterest"].map(s => <span key={s} style={{ fontSize: 13, color: "#C4A99A" }}>{s}</span>)}</div>
          </div>
        </div>
      </footer>
    </div>
